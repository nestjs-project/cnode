...
export class AuthController {
    ...
    async loginView(@Req() req: Request) {
        const error: string = (req as any).flash('loginError')[0];
    ...
    async passportLocal(@Req() req: Request, @Res() res: Response) {
        ...
        this.verifyLogin(req, res, (req as any).user as UserEntity);
    }
    ...
    async githubCallback(@Req() req: Request, @Res() res: Response) {
        ...
        const existUser = await this.authService.github((req as any).user as GitHubProfile);
    ...
    async logout(@Req() req: Request, @Res() res: Response) {
        // 销毁 session
        (req as any).session.destroy(null);
        // 清除 cookie
